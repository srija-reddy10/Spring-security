...
//                    auth.requestMatchers("/crudOperations/addProduct", "/crudOperations/addProducts", "/crudOperations/update", "/crudOperations/delete/{id}")
//                            .hasAuthority("ROLE_ADMIN"); // Ensuring these endpoints are accessible only by ADMIN
//                    auth.anyRequest().authenticated();
//                })
//                .oauth2Login(oauth2 -> oauth2.userInfoEndpoint().userService(oAuth2UserService()))
//                .formLogin(withDefaults())
...
//    }
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http.csrf().disable()
                .authorizeHttpRequests(auth -> {
                    auth.requestMatchers("/crudOperations/welcome", "/details/new").permitAll();
                    auth.requestMatchers(HttpMethod.GET, "/crudOperations/products", "/crudOperations/productById/{id}", "/crudOperations/productByName")
                            .hasAuthority("ROLE_USER");
                    auth.requestMatchers(HttpMethod.POST, "/crudOperations/addProduct", "/crudOperations/addProducts")
                            .hasAuthority("ROLE_ADMIN");
                    auth.requestMatchers(HttpMethod.PUT, "/crudOperations/update")
                            .hasAuthority("ROLE_ADMIN");
                    auth.requestMatchers(HttpMethod.DELETE, "/crudOperations/delete/{id}")
                            .hasAuthority("ROLE_ADMIN");
                    auth.anyRequest().authenticated();
                })
                .oauth2Login(oauth2 -> oauth2.userInfoEndpoint().userService(oAuth2UserService()))
...
//    }
    ...
    public OAuth2UserService<OAuth2UserRequest, OAuth2User> oAuth2UserService() {
        return new CustomOAuth2UserService(); // Your custom OAuth2 user service
    }
